function out = xyz_registration_fitting_6dof(Qcmd, Xmeas)
...
P_star = lsqnonlin(@(P)residual_xyz_reg6(P, Qcmd, Xmeas), P0, lb, ub, opts);
...
r = residual_xyz_reg6(P_star, Qcmd, Xmeas);
out.resnorm = sum(r .^ 2);
out.rmse = sqrt(out.resnorm / numel(r));
out.rmse_xyz = rmse_xyz_from_r(r);
end

function r = residual_xyz_reg6(P, Qcmd, Xmeas)
roll = P(1);
pitch = P(2);
yaw = P(3);
t = P(4:6);
